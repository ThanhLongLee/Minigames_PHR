...
namespace Web.Client.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> GetStatusAddScore(long userId)
        {
            try
            {
                // Gọi service getStatusAddScore để kiểm tra trạng thái cộng điểm
                var result = await _userWallet.getStatusAddScore(userId);

                // Trả về kết quả (1 nếu đã cộng điểm, 0 nếu chưa)
                return Json(result);
            }
            catch (Exception ex)
            {
