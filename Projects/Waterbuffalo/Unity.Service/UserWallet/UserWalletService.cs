    ...
    public partial class UserWalletService : BaseService<UserWalletConnection>, IUserWallet
    {
        ...
        public async Task<long> Connect(long userId, string walletAddress, string exchangeName)
        {
            try
            {
                ...
                var result = await Task.FromResult(ExecuteSql("ConnectUserWallet_New", Params.Create(arr)));
            }
            catch (Exception ex)
            {
        ...
        public async Task<UserWalletConnection> getStatusAddScore(long userId)
        {
            try
            {
                ...
                return await Task.FromResult(base.SqlQuery("CheckUserPointsStatus", Params.Create(arr)).Single());
            }
            catch (Exception ex)
            {
                base.WriteError("Error disconnect user wallet", ex.Message);             
